...
`unpack` (extract from OCI layout or registry) · `versions <ns>/<id>` (list published versions).
Registered in `cmd/root.go`; one file per command (`publish.go`, `verify.go`, …). (`regtoken.go`
is an internal helper — `ensureRegistryToken()` — not a user command.)
...
`digest` (SHA256) · `cache` (immutable-tag disk cache). Imports `grc-store-protocol`
(discovery, syncapi, registrytoken, spdx).
...
- Config: `.grcli.yaml` (or `$XDG_CONFIG_HOME/grcli/config.yaml`, `$HOME/.grcli.yaml`); env prefix `GRCLI_*` (e.g. `GRCLI_REGISTRY_TOKEN`, `GRCLI_GEMARA_SPEC_DIR`).
